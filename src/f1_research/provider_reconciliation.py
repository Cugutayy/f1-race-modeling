...
schema. Hard classification fields must agree. Secondary fields are compared only when
both providers actually expose evidence; missing evidence remains UNKNOWN.
...
def _clean_text(value: Any) -> str | None:
    """Normalize provider text without ambiguous pandas-null truth coercion."""
    if value is None:
        return None
    try:
