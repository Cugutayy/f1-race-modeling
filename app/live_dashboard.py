...
def recent_telemetry(path: Path, driver_number: int, max_rows: int = 1400) -> pd.DataFrame:
    if not path.exists():
        return pd.DataFrame()
    # Captures are append-only. For a personal research app the bounded tail is enough;
    # production storage should use an indexed event store rather than rereading JSONL.
    lines = path.read_text(encoding="utf-8", errors="replace").splitlines()[-20_000:]
    rows = []
    for line in reversed(lines):
        ...
        rows.append({
            ...
            "speed_kmh": payload.get("speed"), "throttle_pct": payload.get("throttle"),
            "brake": payload.get("brake"), "rpm": payload.get("rpm"),
            "gear": payload.get("n_gear"), "drs": payload.get("drs"),
        })
...
def leaderboard(state: dict, predictions: list[dict] | None = None) -> pd.DataFrame:
    ...
    if frame.empty:
        return frame
    keep = [c for c in ("position", "acronym", "driver_number", "team_name", "gap_to_leader_s",
                        "interval_s", "compound", "tyre_age", "last_lap_s", "pit_stops",
                        "speed_kmh") if c in frame]
    frame = frame[keep].copy()
    if predictions:
        p = pd.DataFrame(predictions)[["driver_number", "win_probability", "podium_probability",
                                       "expected_position", "position_p10", "position_p90"]]
        frame = frame.merge(p, on="driver_number", how="left")
...
def prediction_chart(predictions: pd.DataFrame) -> alt.Chart:
    ...
    return alt.Chart(chart).mark_bar().encode(
        ...
        tooltip=["label:N", alt.Tooltip("win_probability:Q", format=".1%"),
...
def location_chart(state: dict) -> alt.Chart | None:
    ...
    labels = alt.Chart(frame).mark_text(dx=12, fontSize=11).encode(x="x:Q", y="y:Q", text="label:N")
...
def telemetry_charts(frame: pd.DataFrame) -> None:
    if frame.empty:
        ...
        return
    for column, title in (("speed_kmh", "Speed · km/h"), ("throttle_pct", "Throttle · %"),
                          ("brake", "Brake · 0/100")):
        valid = frame.dropna(subset=[column])
        if valid.empty:
            continue
        st.altair_chart(alt.Chart(valid).mark_line().encode(
            x=alt.X("date:T", title=None), y=alt.Y(f"{column}:Q", title=title),
    ...
    if state_time and state_time != st.session_state.last_probability_state:
        for row in predictions:
            st.session_state.probability_history.append({
                "time": state_time, "driver": row["label"], "win_probability": row["win_probability"]})
...
def main() -> None:
    ...
    with st.sidebar:
        state_path = Path(st.text_input("State file", os.environ.get("F1_LIVE_STATE_PATH", str(DEFAULT_STATE))))
        total_laps = st.number_input("Race total laps", min_value=2, max_value=100, value=57, step=1)
        samples = st.select_slider("Live simulation samples", options=[2000, 4000, 8000, 12000, 20000], value=4000)
    ...
    def live_view():
        ...
        age = float(state.get("data_age_s") or 0)
        ...
        report = None
        try:
            report = predict_from_state(state, int(total_laps), config=SimulationConfig(samples=int(samples)))
        ...
        with race:
            ...
            cols = st.columns(4)
            cols[0].metric("Track °C", weather.get("track_temperature_c") or "—")
            cols[1].metric("Air °C", weather.get("air_temperature_c") or "—")
            cols[2].metric("Rain", "YES" if weather.get("rainfall") else "No")
            cols[3].metric("SC / flag", state.get("safety_car") or state.get("flag") or "Green/unknown")
        ...
        with probability:
            if not predictions:
                ...
                if not history.empty:
                    ...
                    st.altair_chart(alt.Chart(history).mark_line(point=False).encode(
                        x=alt.X("time:T", title=None),
                        y=alt.Y("win_probability:Q", title="Win probability", axis=alt.Axis(format="%")),
                        color=alt.Color("driver:N", title="Driver"),
                        tooltip=["driver:N", alt.Tooltip("time:T"),
                                 alt.Tooltip("win_probability:Q", format=".1%")],
                    ).properties(height=320), width="stretch")
                st.caption("Probabilities are coherent Monte Carlo outcomes from the current public-data state; they are not team probabilities or betting odds.")
        ...
        with track:
            ...
            if chart is None:
                ...
                st.caption("OpenF1 x/y location is approximate track position, not precision GPS or lateral racing-line data.")

        with telemetry:
            drivers = [(d.get("acronym") or str(d["driver_number"]), int(d["driver_number"]))
            ...
            if not drivers:
                ...
                st.caption("Public car_data is ~3.7 Hz broadcast telemetry. Brake is binary 0/100; this is not the team's full sensor feed.")

        with strategy:
            drivers = [(d.get("acronym") or str(d["driver_number"]), int(d["driver_number"]))
                       for d in state.get("drivers", []) if d.get("position")]
            if not predictions or not drivers:
                ...
                if scenarios:
                    ...
                    st.altair_chart(alt.Chart(table).mark_line(point=True).encode(
                        ...
                        color="compound:N", tooltip=["compound:N", "pit_in_laps:Q",
                                                     alt.Tooltip("expected_position:Q", format=".2f"),
                                                     alt.Tooltip("win_probability:Q", format=".1%")],
                    ).properties(height=300), width="stretch")
                    st.caption("Counterfactual sensitivity analysis: lower expected position is better. Defaults must be validated circuit-by-circuit before stronger claims.")

        with audit:
            st.json({
                "state": {key: state.get(key) for key in ("session_key", "meeting_key", "updated_at",
                                                          "snapshot_at", "data_age_s", "received_messages",
                                                          "rejected_stale_messages")},
                "simulation": report.get("audit") if report else None,
            }, expanded=False)
            st.warning("Missing team-only variables (fuel, setup, carcass/internal tyre temperatures, full sensor channels) are never imputed as if observed.")
